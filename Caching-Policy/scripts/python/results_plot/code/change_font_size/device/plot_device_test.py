...
result_dir = '../../../result/change_font_size/device'
statistic_name = 'statistic.xlsx'
device_id_list = ['eMMC', 'SD']
operation_read_ratio_list = ['read_0', 'read_1']
operation_read_ratio_dict = {'read_0': 'Pure Write', 'read_1': 'Pure Read'}
...
def barplot_pure_read_vs_pure_write(device_id_, y_label_):
    ...
    y_label = f'{y_label_} of {device_id_}'
    ax.set_ylabel(y_label, weight='bold', fontsize=16)
    ax.set_xticks([idx * 2 + 0.2 for idx in range(len(block_sizes))])
    ax.set_xticklabels(block_sizes)
    # 设置图例
    ax.legend(['Pure Read', 'Pure Write'], loc='upper center', bbox_to_anchor=(0.5, 1.1), fancybox=True,
              shadow=True, ncol=2, fontsize=14, frameon=False)
...
def barplot_emmc_vs_sd(operation_read_ratio_, y_label_):
    ...
    # 设置横纵坐标标签和标题
    ax.set_xlabel('Block Size(KB)', weight='bold', fontsize=16)
    y_label = f'{y_label_} of {operation_read_ratio_dict[operation_read_ratio_]}'
    ax.set_ylabel(y_label, weight='bold', fontsize=16)
    ax.set_xticks([idx * 2 + 0.2 for idx in range(len(block_sizes))])
    ax.set_xticklabels(block_sizes)
    # 设置图例
    ax.legend(['eMMC', 'SD'], loc='upper center', bbox_to_anchor=(0.5, 1.1), fancybox=True,
              shadow=True, ncol=2, fontsize=14, frameon=False)
