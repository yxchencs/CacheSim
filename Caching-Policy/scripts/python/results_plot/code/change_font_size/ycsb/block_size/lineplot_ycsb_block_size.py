...
result_dir = '../../../../result/change_font_size/ycsb/block_size/lineplot'
...
def lineplot_ycsb_caching_policy(combination_, y_label_):
    ...
    ax.set_xticklabels(discrete_ticks, fontproperties=font_prop)

    # 设置坐标轴标签
    ax.set_xlabel("Block Size(KB)", weight='bold', fontsize=16, fontproperties=font_prop)
    ax.set_ylabel(f"{y_label_}", weight='bold', fontsize=16, fontproperties=font_prop)

    # ax.legend(loc='best', ncol=3, frameon=False, handlelength=3.5, fontsize=16, prop=font_prop)
    # ax.legend(caching_policy_list, loc='upper center', bbox_to_anchor=(0.5, 1.15), fancybox=True, shadow=False, ncol=5, frameon=False, fontsize=16)
