...
result_dir = '../../../../result/change_font_size/ycsb/operation_read_ratio/lineplot'

# 读入数据
df = pd.read_excel(os.path.join(data_dir, 'statistic_ycsb_125MB_all.xlsx'))
...
def lineplot_ycsb_caching_policy(combination_, y_label_):
    ...
    trace_data = df[(df['Workload Type'] == workload_type) & (df['Block Size(KB)'] == block_size) & (
    ...
    # 设置 x 轴的离散值
    # discrete_ticks = df['Operation Read Ratio'].unique()
    discrete_ticks = [0, 0.2, 0.4, 0.6, 0.8, 1]
    ...
    for idx, caching_policy in enumerate(caching_policy_list):  # 筛选特定 Caching Policy 的数据
        ...
        # 过滤掉不在离散值范围内的行
        policy_data = policy_data[policy_data['Operation Read Ratio'].isin(discrete_ticks)]
        ...
        # 绘制折线图
        sns.lineplot(ax=ax, data=policy_data, x='Operation Read Ratio', y=f"{y_label_}", marker=mk, linestyle=ls,
    ...
    # 设置坐标轴标签
    ax.set_xlabel('Operation Read Ratio', weight='bold', fontsize=16, fontproperties=font_prop)
    ...
    ax.legend(caching_policy_list, loc='upper center', bbox_to_anchor=(0.5, 1.17),
              fancybox=True, shadow=False, ncol=5, frameon=False, fontsize=14, prop=font_prop)
