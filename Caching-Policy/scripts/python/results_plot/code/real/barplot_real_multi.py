...
trace_operation_combinations = original_data[
    ['Workload Type', 'Real Trace Name', 'IO(on/off)']].drop_duplicates()
...
y_labels = ['Hit Ratio', 'Average Latency(ms)', 'P99 Latency(ms)', 'Average CPU Usage(%)', 'Total Time(s)',
            'Bandwidth(MB/s)', 'Average Power(W)', 'Energy(J)']


def barplots_real_cache_size_and_policy(combination_, y_label_):
    workload_type, real_trace_name, io_status = combination_
    if y_label_ == 'Bandwidth(MB/s)':
        file_name = 'Bandwidth(MBps)'
    else:
        file_name = y_label_.replace(' ', '_')
    save_dir = os.path.join(result_dir, workload_type, real_trace_name, io_status)
