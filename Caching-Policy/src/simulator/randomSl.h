...
void RandomSl::writeCache(const long long &key)
{
    if (!isWriteCache())
        return;
    ...
    { // cache full
        ...
        int key_erased = -1;
        key_erased = cache_map.Touch(key);
...
void RandomSl::test()
{
    ...
    if (!fin.is_open())
    {
        ...
        cout << "Error: opening trace file fail, errno = " << errno << endl;
        exit(1);
    }
    getline(fin, s);
    // getline(fin, s);
